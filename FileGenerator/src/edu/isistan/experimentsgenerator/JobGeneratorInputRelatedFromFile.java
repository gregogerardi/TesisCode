...
import edu.isistan.jobs.JobGeneratorInputRelated;
import edu.isistan.jobs.JobInformation;
...
public class JobGeneratorInputRelatedFromFile {
    ...
    private void generateFile(String line, String outputRootFolder) {
        ...
        String outputPath = args[9];//given the parameter format ParamName:Function, instantiate the opsFunction variable with Function value
        ...
        String outputFile = outputRootFolder + "\\" + outputPath;
        File file = new File(outputFile);
        File file2 = file;
        LinkedList<String> parents = new LinkedList<>();
        while (!file2.exists()) {
            parents.addFirst(file2.getParentFile().getName());
            file2 = file2.getParentFile();
        }
        if (!parents.isEmpty()) parents.removeFirst();
        while (!file2.getAbsolutePath().equals(file.getParentFile().getAbsolutePath())) {
            file2 = new File(file2.getAbsolutePath() + "\\" + parents.removeFirst());
            file2.mkdir();
        }
    ...
    private long getOps(String opsFunction, long inputEntries) {
        ...
        if (opsFunction.compareTo("nlogn") == 0)
            return (long) Math.log(inputEntries) * inputEntries;
        ...
        if (opsFunction.startsWith("n_")) {
            ...
            return (long) Math.pow(inputEntries, exp);
        }
        ...
        if (opsFunction.compareTo("all") == 0) {
            ...
            switch (function) {
                case 0:
                    return (long) Math.log(inputEntries) * inputEntries;
                case 1:
                    return (long) Math.pow(inputEntries, 2);
                case 2:
                    return (long) Math.pow(inputEntries, 3);
            }
        }
        return -1;
    }
